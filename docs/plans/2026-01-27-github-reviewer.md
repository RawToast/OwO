...
```json
{
  "github-review": {
    ...
    "reviewers": [
      {
        "name": "security",
        "prompt": "You are a security-focused code reviewer. Look for vulnerabilities, auth issues, injection risks.",
        "focus": "security"
      },
      {
        "name": "quality", 
        "prompt": "You are a helpful peer reviewer. Focus on code quality, readability, and best practices.",
        "focus": "quality"
      }
    ],
    "overview": {
      "prompt": "Synthesize the reviewer findings into a cohesive summary. Be helpful and constructive.",
      "includeDiagram": true,
      "includeTable": true
    }
  }
...
*Reviewed by [owo-reviewer](https://github.com/jmagar/owo) • 2 reviewers • 4 inline comments*
```
...
3. Post the review via GitHub API`,
  args: {
    overview: tool.schema
      .string()
      .describe("Markdown overview comment summarizing the review. Will appear at the top of the review."),
    comments: tool.schema
      .array(InlineCommentSchema)
      .default([])
      .describe("Inline comments on specific lines. Each needs path, line number, and comment body."),
    event: tool.schema
...
**Step 1: Add GitHub review config schema**
 ...
 */
export const GitHubReviewerConfigSchema = z.object({
  name: z.string().describe("Reviewer identifier (e.g., 'security', 'quality')"),
  prompt: z.string().optional().describe("Custom prompt for this reviewer's persona"),
 ...
 */
export const GitHubReviewOverviewConfigSchema = z.object({
  prompt: z.string().optional().describe("Prompt for synthesizing the overview"),
...
export const GitHubReviewConfigSchema = z.object({
  ...
  trigger: z.enum(["auto", "command"]).optional().default("command")
    .describe("'auto' reviews all PRs, 'command' waits for /review"),
  mentions: z.array(z.string()).optional().default(["/review", "/oc-review"])
    .describe("Trigger phrases for command mode"),
  reviewers: z.array(GitHubReviewerConfigSchema).optional().default([
    { name: "default", focus: "general code quality" }
...
const sampleDiff = readFileSync(
  join(import.meta.dir, "fixtures/sample.diff"),
  "utf-8"
)
